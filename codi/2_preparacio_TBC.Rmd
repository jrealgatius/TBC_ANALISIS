...
ñ<-dt_ecap %>% select(CIP,grup,dtindex,DG.DM) %>% filter(DG.DM>ymd(dtindex) & grup==1)
...
dt_ecap %>% select(CIP,grup,dtindex,DG.DM) %>% filter(DG.DM>ymd(dtindex) & !is.na(DG.DM))
...
save(dt_ecap,dt_agencia, file=here::here(dir_output,"output4.Rdata"))
