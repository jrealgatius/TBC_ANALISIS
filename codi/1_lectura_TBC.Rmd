...
params:
  ...
  - sexe
  - any_naix
  metode: "estatica" # "dinamica" / "estatica" / "PS"
  num_controls: 1
---
...
dt_dades<-dt_dades %>% mutate(DET_TB_ECAP=DET_TB,
                              ...
                              DET_TB=if_else(is.na(DET_TB),DET_TB_ECAP,DET_TB))
...
# 5.2. Generar data_index segons primer DM detectat en problemes de salut durant reclutament  --------
dt_dtindexDM<-
...
dt_antecedents.agregada2<-agregar_problemes(dt_psalut,bd.dindex =data_index_agregacio,dt.agregadors=CATALEG,
                                           finestra.dies = c(-Inf,+1),prefix="DG.",camp_agregador="subgrup") 
...
dt_antecedents.agregada3<-agregar_problemes(dt_psalut,bd.dindex =data_index_agregacio,dt.agregadors=CATALEG,
                                          finestra.dies = c(-Inf,+1),prefix="DG.",camp_agregador="DM")
...
dt_farmacs_agregada<-
  agregar_prescripcions(dt=dt_farmacia,bd.dindex=data_index_agregacio,dt.agregadors=CATALEG,prefix="FP.",finestra.dies=c(-45,+45),camp_agregador="grup",agregar_data=F) %>% 
  mutate(dtindex=data.to.string(dtindex))

temp<-
  agregar_prescripcions(dt=dt_farmacia,bd.dindex=data_index_agregacio,dt.agregadors=CATALEG,prefix="FP.",finestra.dies=c(-45,+45),camp_agregador="subgrup",agregar_data=F) %>% 
  mutate(dtindex=data.to.string(dtindex))
