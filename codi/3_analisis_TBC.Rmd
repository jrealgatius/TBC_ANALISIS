...
# Salvar taula de taxes

res1 %>% bind_rows(res2) %>% bind_rows(res3) %>% bind_rows(res4) %>% 
  saveRDS("resultats/incidencies.Rds")
...
saveRDS(taula_models,"resultats/taula_models.Rds")
...
mom5.1 %>% bind_rows(mom5.2) %>% bind_rows(mom5.3) %>% 
  saveRDS("resultats/HR_subtipus.Rds")
...
forest.plot.modelcomplet(dadesmodel,label="Categoria",mean="HR",lower="Li95%CI",upper="Ls95%CI",label_X="HR (95% CI)", intercept=1,add_table = T)+
  theme_minimal(base_size = 12)

pp<-forest.plot.modelcomplet(dadesmodel,label="Categoria",mean="HR",lower="Li95%CI",upper="Ls95%CI",label_X="HR (95% CI)", intercept=1,add_table = T)+
  theme_minimal(base_size = 12) + 
...
saveRDS(modelcomplet,"resultats/modelcomplet.Rds")
