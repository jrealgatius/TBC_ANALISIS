...
params:
  metode: "dinamica"  # "dinamica" / "estatica" / "PS"
  subtitul: ""
...
&check; Agrupació de paisos en Árees <br/> 
&check; Anàlisis de sensibilidad amb esdeveniments ASPB <br/>
&check; + Models <br/>
&check; Descriptiu d'esdeveniments de TBC <br/>
&check; Descriptiu de dades d ' <br/>
...
## Incidència segons dades (TBC només confirmats segons dades agència)

```{r resultats6, message=FALSE, warning=FALSE, include=T, echo=FALSE,size="huge"}

kable(resum_events(dades = dades,evento="event_tbc_agencia",temps="anys_lliure_tbc",valorevent = "1"),caption="Resum d'events") %>% kableExtra::kable_styling()

dades %>% base::split(dades$grup) %>% 
  map(~resum_events(dades = .x,evento="event_tbc_agencia",temps="anys_lliure_tbc",valorevent = "1")) %>% 
  map(as_tibble) %>% 
  map_df(bind_rows,.id = "grup") %>% 
  as_tibble()

kable(taula_IA_grup,caption="Resum d'events per exposició a diabetis") %>% kableExtra::kable_styling()
descrTable(surv_tbc_agencia~grup,data=dades,show.ratio = T, byrow = T ) %>% export2md(caption = "Incidència i Hazard Ratio de Tuberculosis")
...
if (params$metode=="dinamica") cluster="case.id" else cluster=""
...
## Supervivencia lliure d'esdeveniment K-M (TBC confirmat Agencia)

```{r figura2, include=T, fig.asp=1}
survfit(surv_tbc_agencia ~ grup, data=dades) %>%
  ggsurvplot(title="TB detection in patient with or without DM",
...
dades %>% mutate(any_DM=year(DG.DM),any_DM=ifelse(any_DM<=2007,"<=2007",as.character(any_DM))) %>% 
  ...
  kable(digits = 3,caption = "Diabètics a 2007 i incidents per any durant seguiment") %>% kableExtra::kable_styling()
```
...
# Estudi exploratori dades de l'Agencia

> Descriptiu general del tots els casos de TBC ja siguin antencedents o events de la base de dades de l'Agència
> Tots els casos amb CIP inclosos en estudi previ

```{r filtre_agencia,include=F}
...
dt_agencia %>% select(CIP,DET_TB,event_tbc) %>% 
  summarise(n(),n_distinct(CIP), sum(event_tbc=="Si")) %>% 
  kable(digits = 3,caption = "Casos de TBC de la base de dades de l'Agencia") %>% kableExtra::kable_styling()
...
# Descriptiu nomes events del subestudi


```{r}

dt_temp<-dt_agencia %>% filter(event_tbc=="Si")
