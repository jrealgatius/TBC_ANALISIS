...
# Relevel 
dades<- dades %>% mutate(grup=relevel(grup, "Control"))
...
export2md(T1_BASAL,caption = "Descriptiva de caracterítiques basals entre grups (Mitjana (SD))")
...
```{r analisis5, message=FALSE, warning=FALSE, include=T, echo=FALSE,size="huge"}
# T BASAL -------------
...
> Descriptiu general dels casos de TBC ja siguin antencedents o events de la base de dades de l'Agència
> Casos amb CIP inclosos en subestudi
...
# Ara filtro amb events de  subestudi que siguin de 
dt_agencia<-dt_agencia %>% semi_join(dades, by="CIP") %>% filter(dt_agencia==1)
...
descrTable(formula,data=dt_agencia,max.xlev = 100,show.n = T,hide.no="NO",simplify=F,method = 2, Q1=0,Q3=1) %>% 
