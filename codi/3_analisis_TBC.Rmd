...
descrTable(formu,data=dt_temp,show.p.overall = F,show.n = T, hide.no = T,hide = "No") %>% 
  export2md(caption = "Baseline characteristics of patients in diabetic cohort")
...
res1<-dades %>% Resum_taxa_incidencia(evento="event_tbc",temps="anys_lliure_tbc",valorevent="1") %>% mutate(Grup="Overall")

res2<-dades %>% split(.$grup) %>% map_dfr(~Resum_taxa_incidencia(.x,evento="event_tbc",temps="anys_lliure_tbc",valorevent="1"),.id = "Grup") 

res3<-dades %>% split(.$grup2) %>% map_dfr(~Resum_taxa_incidencia(.x,evento="event_tbc",temps="anys_lliure_tbc",valorevent="1"),.id = "Grup") 
res4<-dades %>% split(.$grup3) %>% map_dfr(~Resum_taxa_incidencia(.x,evento="event_tbc",temps="anys_lliure_tbc",valorevent="1"),.id = "Grup") 

res1 %>% bind_rows(res2) %>% bind_rows(res3) %>% bind_rows(res4) %>% 
  kable(digits = 2,caption = "Table3. Incidence of TB in DM and non-DM patients ") %>% kableExtra::kable_styling()
...
- No ajustats, + ajustos segons 2 models
...
taula_models_COX<- mod1 %>% bind_rows(mod2) %>% bind_rows(mod3) %>% bind_rows(mod4) 

# Model de riscos (dades, covariables)

mod1<-Estima_HR_RCrisk_clusters(dades,"",a="grup",failcode = "Event",cencode = "End of follow-up") %>% mutate(adjust="Unadjusted")

mod2<-Estima_HR_RCrisk_clusters(dades,"DM_ajust",a="grup",failcode = "Event",cencode = "End of follow-up") %>% mutate(adjust="Model 1")

mod3<-Estima_HR_RCrisk_clusters(dades,"DM_ajust2",a="grup",failcode = "Event",cencode = "End of follow-up") %>% mutate(adjust="Model 2")

mod4<-Estima_HR_RCrisk_clusters(dades,"DM_ajust3",a="grup",failcode = "Event",cencode = "End of follow-up") %>% mutate(adjust="Model 3")
...
mom5.1<-Estima_HR_RCrisk_clusters(dades,"DM_ajust3",a="grup",failcode = "Event",cencode = "End of follow-up") 
mom5.2<-Estima_HR_RCrisk_clusters(dades,"DM_ajust3",a="grup2",failcode = "Event",cencode = "End of follow-up") 
mom5.3<-Estima_HR_RCrisk_clusters(dades,"DM_ajust3",a="grup3",failcode = "Event",cencode = "End of follow-up") 


mom5.1 %>% 
  bind_rows(mom5.2) %>% bind_rows(mom5.3) %>%  
...
## Figura d'incidencia acumulada de TB  segons K-Meier (TBC)
