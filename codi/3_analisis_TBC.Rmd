...
dades %>% select(DG.TAB_cat,DG.TAB)
...
events_dades_temp<-dades %>% filter(event_tbc==1) %>% select(CIP,event_tbc_cohort=event_tbc)

dt_agencia <- 
  dt_agencia %>% filter(dt_agencia==1) %>% 
  left_join(events_dades_temp) %>% group_by(event_tbc_cohort) 
...
## Descriptiva comparativa de events TVC del subestudi entre DM vs no DM

```{r, include=T}

formula<-formula.text("taula",y="event_tbc",taulavariables = conductor_agencia)

descrTable(formula,data=dt_temp,max.xlev = 100,show.n = T,hide.no="NO",simplify=F, show.p.overall = F) %>% 
