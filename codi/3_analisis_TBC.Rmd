...
&check; Descriptiva comparativa de Diabetics en funció del tractament  (TAULA 2.1) <br/>
...
if (params$test) dir_results<-"resultats/test" else dir_results<-"resultats"
...
flow1 %>% DiagrammeRsvg::export_svg() %>% charToRaw() %>% rsvg::rsvg() %>% 
  png::writePNG("shiny/figures/flow_chart1.png")
...
# Construir criteris per parells de indidivus
dt_temp<-dades %>% group_by(case.id) %>% mutate(filtre_exitus=sum(filtre_exitus),
                                                age=sum(age<18 | is.na(age))) %>% 
  ungroup() %>% select(CIP,case.id,grup,filtre_exitus,age)

flow2<-criteris_exclusio_diagrama(dt_temp,
                           taulavariables = conductor_variables, forma=c("box"),
                           criteris = "exc2",ordre="exc_ordre",
                           etiquetes = "lab_exc",
                           sequencial = T,grups = "grup")

flow2

flow2 %>% DiagrammeRsvg::export_svg() %>% charToRaw() %>% rsvg::rsvg() %>% 
  png::writePNG("shiny/figures/flow_chart2.png")
...
# table(dades$visites_TOT_CATQ)
# table(dades$visites_TOT)
# table(dades$visites_MG)
# 
# table(dades$visites_MG,dades$grup)
...
dades<-dades %>% mutate(
  ...
  censura_tipo=case_when(datafi==lubridate::ymd(20181231)~"End of follow-up",
                     ...
                     datafi<lubridate::ymd(20181231) & exitus==0 ~"Switch group"))
...
# Trio un parell per tipus de censura (Switch_group,Exitus,End of follow-up)
dt_temp1 <- dt_temp %>% 
  group_by(id) %>% filter(sum(censura_tipo=="End of follow-up")>0) %>% ungroup(id) %>% 
  mostreig_ids("id",2,llavor)

dt_temp2 <- dt_temp %>% 
  group_by(id) %>% filter(sum(censura_tipo=="Switch group")>0) %>% ungroup(id) %>% 
  mostreig_ids("id",2,llavor)
...
pp<-dt_temp %>% 
  ...
  labs(title="Figure 1. Sampling and follow-up scheme",
...
# Salvar a baixa resolució per dashboard
grDevices::png(here::here("codi/shiny/figures","Figure1_esquema.png"))
...
pp2<- pp + 
  ...
  labs(title="Figure 2. Sampling and follow-up scheme",
       subtitle = "5 pairs of individuals by age and sex",
       color="Group:",
       shape="")
...
# Salvar a baixa resolució
grDevices::png(here::here("codi/shiny/figures","Figure2_esquema.png"))
pp2
dev.off()
...
ggsave(here::here(dir_figures,"Figure1.png"),plot=fig1,device="png", units="in", 
       width=7, height=7, dpi=300*2)
...
pp<-dt_temp %>% 
  Esquema_ggplot(datainicial= "data0",  datafinal="dtindex",  id="num",  grup_shape=NA,  grup_color = "Grupo",  lim_inf=-Inf,  lim_sup="20181231",lab_y="lab_SexEdat") +
  labs(title="Figure 3. Sampling and follow-up scheme",
...
# Salvar en alta resolució
grDevices::png(here::here(dir_figures,"Figure1.2_esquema.png"),
               res = 72*resfactor, height=640*resfactor, width=640*resfactor)
pp
dev.off()  

# Salvar en baixa resolució
grDevices::png(here::here("codi/shiny/figures","Figure1.2_esquema.png"))
pp
dev.off()  
...
pp<-pp + 
  ...
  scale_shape(solid=FALSE) +
  annotate("text", x = lubridate::ymd("20171201"), y = 0.5, label = "End of follow-up",size=3,fontface="bold") +
  labs(title="Figure 4. Sampling and follow-up scheme",
       subtitle = "2 pairs of individuals by age and sex",
       color="Group:",
       shape="")
...
# Salvar en baixa resolució
grDevices::png(here::here("codi/shiny/figures","Figure2.2_esquema.png"))
pp
dev.off()  
...
```{r, include=TRUE}
dt_temp<-dades %>% filter(grup=="Diabetes")
...
## TABLE 2.2. Distribució de tractament farmacològic de la diabetis en funció de característiques.

```{r, include=TRUE}

vars<-c("sexe","age","DM_anys","grup2","grup3",extreure.variables("DM_ajust4",conductor_variables))

formu<-formula_vector(vars,y="FP_TRACTAMENT_AD")


descrTable(formu,data=dt_temp,show.p.overall = T,show.n = T, hide.no = T,hide = "No",byrow = T) %>% 
  export2md(caption = "Table  2. Baseline characteristics of patients in diabetic cohort by DM Treatment")
...
taula_incidencies<-res1 %>% bind_rows(res2) %>% bind_rows(res3) %>% bind_rows(res4)
...
HR_subtipus<-mom5.1 %>% bind_rows(mom5.2) %>% bind_rows(mom5.3)

HR_subtipus %>% select(-c(method,adjustedby)) %>% 
...
figure1_survival<-
  plot_cmprisk_cuminc(dt=dades,a="grup",failcode = "Event",cencode = "End of follow-up") + 
  scale_x_continuous(breaks = c(seq(1,3650,730))) +theme_light() +theme(legend.position="top")

figure1_survival

grDevices::png(here::here("figures","Figure_CI.png"),
               res = 72*resfactor*2, height=640*resfactor*2, width=640*resfactor*2)
figure1_survival
dev.off()


ggsave("Figure2.png",plot=figure1_survival,device="png", units="in", width=7, height=7, dpi=600)
...
forest_figura<-forest.plot.HR(taula_models,label="var",mean="HR",lower = "Li95%CI",upper="Ls95%CI",label_X = "Hazard ratio (95% CI)",
               intercept = 1,nolabels=FALSE,
...
taula_models %>% transmute(Model=stringr::str_remove(Model,"Model"),
                                 adjustedby)
...
model<-crrSC::crrc(ftime=dt_temp$temps_tbc,fstatus=dt_temp$status,cov1=(dt_temp$grup=="Diabetes"),cluster=dt_temp$case.id, failcode = "Event", cencode = "Fin estudio")
summary(model)

# ImpPUTAció
pp<-with(imp,crrSC::crrc(ftime=dt_temp$temps_tbc,fstatus=dt_temp$status,cov1=(dt_temp$grup=="Diabetes"),cluster=dt_temp$case.id, failcode = "Event", cencode = "Fin estudio"))
...
dadesmodelcomplet<-modelcomplet$resum_crrsc %>% 
...
forest_model_figura<-forest.plot.modelcomplet(dadesmodelcomplet,label="Categoria",mean="HR",lower="Li95%CI",upper="Ls95%CI",label_X="HR (95% CI)", intercept=1,add_table = T)+
  theme_minimal(base_size = 12) + 
  scale_y_continuous(trans = "log", breaks = c(0.5,1,3,10)) +
  labs(title = "Forest plot of hazard ratios", subtitle = "Complete model")+
  expand_limits(y=c(0.10,150))
...
ggsave("Figure3.png",plot=forest_model_figura ,device="png", units="in", width=7, height=7, dpi=600)
...
dadesmodelcomplet2<-modelcomplet$resum_crrsc %>% 
  filter(!stringr::str_ends(var,"None")) %>%  # Eliminar categoria None 
  mutate(id=n():1,Categoria=stringr::str_sub(var,1,30)) %>% 
  dplyr::select(id,Categoria,HR,`Li95%CI`,`Ls95%CI`,'p-value')

dadesmodelcomplet2 %>% select(-id) %>% 
  kable(digits = 3) %>% kableExtra::kable_styling()


forest.plot.modelcomplet(dadesmodelcomplet2,label="Categoria",mean="HR",lower="Li95%CI",upper="Ls95%CI",label_X="HR (95% CI)", intercept=1,add_table = T)+
  theme_minimal(base_size = 12)

forest_model2_figura<-forest.plot.modelcomplet(dadesmodelcomplet2,label="Categoria",mean="HR",lower="Li95%CI",upper="Ls95%CI",label_X="HR (95% CI)", intercept=1,add_table = T)+
  theme_minimal(base_size = 12) + 
  scale_y_continuous(trans = "log", breaks = c(0.5,1,3,10)) +
  labs(title = "Forest plot of hazard ratios", subtitle = "Complete model")
...
```{r salvar_objectes}
# Salvaré taules de resultats i figures per tal de després graficar-les en format Dashboard

# Analisis de sensibilitat 
if (params$test) output_shiny<-"output_test.RData" else output_shiny<-"output.RData"
...
save(taula_models,
     ...
     dadesmodelcomplet2,
     forest_model2_figura,
  file = here::here("codi/shiny",output_shiny))
