...
grDevices::png(here::here("figures","Figure1_esquema.png"))
...
grDevices::png(here::here("figures","Figure2_esquema.png"))
pp
dev.off()
...
# dt_temp %>% group_by(id) %>% filter(sum(censura_tipo=="Swich group")>0) %>% ungroup(id) 
# 
# c("MAFI152062900","LOGA039032200")
# 
# dt_temp %>% group_by(id) %>% filter(CIP %in% c("MAFI152062900","LOGA039032200")) %>% ungroup()
# 
# dades %>% filter(CIP %in% c("MAFI152062900","LOGA039032200"))
...
set.seed(123)
# ids<-dt_temp %>% pull(id) %>% unique() %>% sample (5)
