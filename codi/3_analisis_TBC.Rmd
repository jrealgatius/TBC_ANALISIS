...
mod1<-extreure_HR(a="grup",x="",event = "event_tbc",t="temps_tbc",d=dades,taulavariables = conductor_variables,c=cluster) %>% mutate(adjust="Unadjusted")

mod2<-extreure_HR(a="grup",x="DM_ajust",event = "event_tbc",t="temps_tbc",d=dades,taulavariables = conductor_variables,c=cluster) %>% mutate(adjust="Model 1")

mod3<-extreure_HR(a="grup",x="DM_ajust2",event = "event_tbc",t="temps_tbc",d=dades,taulavariables = conductor_variables,c=cluster) %>% mutate(adjust="Model 2")

mod4<-extreure_HR(a="grup",x="DM_ajust3",event = "event_tbc",t="temps_tbc",d=dades,taulavariables = conductor_variables,c=cluster) %>% mutate(adjust="Model 3")


# guardar resum models
taula_models_COX<- mod1 %>% 
  bind_rows(mod2) %>% 
  bind_rows(mod3) %>% 
  bind_rows(mod4) 
...
mod1<-Estima_HR_RCrisk_clusters(dades,"",a="grup") %>% mutate(adjust="Unadjusted")

mod2<-Estima_HR_RCrisk_clusters(dades,"DM_ajust",a="grup") %>% mutate(adjust="Model 1")

mod3<-Estima_HR_RCrisk_clusters(dades,"DM_ajust2",a="grup") %>% mutate(adjust="Model 2")

mod4<-Estima_HR_RCrisk_clusters(dades,"DM_ajust3",a="grup") %>% mutate(adjust="Model 3")

# dades %>% select(grup,grup2)
# printar models
taula_models<-mod1 %>% bind_rows(mod2) %>% bind_rows(mod3) %>% bind_rows(mod4) %>% bind_rows(taula_models_COX)
  
taula_models %>%  kable(digits = 3,caption = "HR de risk de TB segons diferents models ajustats ") %>% 
...
## Forest plot analisis de sensibilitat
...
forest.plot.HR(taula_models,label="var",mean="HR",lower = "Li95%CI",upper="Ls95%CI",label_X = "Hazard ratio (95% CI)",
               ...
               nivell = "adjust",factor1="method",label_Xvertical="Fit",
               label_Favors="Favors Controls        Favors DM",color = T)

forest.plot.HR(taula_models,label="var",mean="HR",lower = "Li95%CI",upper="Ls95%CI",label_X = "Hazard ratio (95% CI)",
               intercept = 1,nolabels=FALSE,
               nivell = "method",factor1="adjust",label_Xvertical="Fit",
               label_Favors="Favors Controls        Favors DM",color = T)
...
mod1<-extreure_HR(a="grup2",x="",event = "event_tbc",t="temps_tbc",d=dades,taulavariables = conductor_variables,c=cluster)

mod2<-extreure_HR(a="grup2",x="DM_ajust",event = "event_tbc",t="temps_tbc",d=dades,taulavariables = conductor_variables,c=cluster)

mod3<-extreure_HR(a="grup2",x="DM_ajust2",event = "event_tbc",t="temps_tbc",d=dades,taulavariables = conductor_variables,c=cluster)

mod4<-extreure_HR(a="grup2",x="DM_ajust3",event = "event_tbc",t="temps_tbc",d=dades,taulavariables = conductor_variables,c=cluster)
